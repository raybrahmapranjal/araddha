...
if(isset($_POST['submit'])){
    ...
    if (empty($_FILES["aboutImage"]["name"])) {
        ...
        $count= $crud ->Count("slider","1");
       
        if($count<1){
            $create = $crud->Create($data,"slider");

            if($create) {
                $_SESSION['message']= '<script>alert("Data Uploaded!");</script>';
                echo '<script>window.location.assign("../view_slider.php");</script>';
            }
        } else {
        ...
        if ($uploadOk == 0) {
          echo '<script>alert("Sorry, your file was not uploaded.");</script>';
           echo '<script>window.location.assign("../catalog/view_slider.php");</script>';
            ...
            if (move_uploaded_file($_FILES["aboutImage"]["tmp_name"], "../".$target_file)) {
              ...
              // $_SESSION['message']= "The file ". basename( $_FILES["aboutImage"]["name"]). " has been uploaded.";
                ...
                if($count<1){
                    ...
                    $update =$crud->Create($data,"slider");
